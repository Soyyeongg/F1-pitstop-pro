...
def get_race_telemetry(session, session_type="R"):
    ...
    # Find Verstappen (VER) only
    drivers = session.drivers
    
    if not drivers or len(drivers) == 0:
        raise ValueError("No drivers found in this session. The session data may not be available yet for this year/round. Try a different year (e.g. 2024).")
    
    driver_codes = {num: session.get_driver(num)["Abbreviation"] for num in drivers}
    
    verstappen_driver_no = None
    for driver_no in drivers:
        if driver_codes[driver_no] == 'VER':
            verstappen_driver_no = driver_no
            break
    
    if verstappen_driver_no is None:
        print("Verstappen not found in this session, using first available driver")
        verstappen_driver_no = drivers[0]

    print(f"Processing only {driver_codes[verstappen_driver_no]} - single driver")
    
    # Process Verstappen's telemetry data directly
    result = _process_single_driver((verstappen_driver_no, session, driver_codes[verstappen_driver_no]))
    
    if result is None:
        raise ValueError("No valid telemetry data found for Verstappen")

    code = result["code"]
    driver_data = {code: result["data"]}
    
    global_t_min = result["t_min"]
    global_t_max = result["t_max"]
    max_lap_number = result["max_lap"]
    ...
    #4.2. Aggregating Verstappen's pit-in and pit-out data only
    pit_windows = {}
    laps = session.laps
    
    # Only process Verstappen
    drv = driver_codes[verstappen_driver_no]
