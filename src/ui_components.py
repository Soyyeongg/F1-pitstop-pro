...
class LeaderboardComponent(BaseComponent):
    ...
    def draw(self, window):
        ...
        # SINGLE DRIVER MODE: Only show Verstappen (VER)
        verstappen_entries = []
        for entry in self.entries:
            code, color, pos, progress_m = entry
            if code == 'VER':
                verstappen_entries.append(entry)
                break  # Only need the first (and only) VER entry
        
        # If no VER found, take the first driver (fallback)
        if not verstappen_entries and self.entries:
            verstappen_entries = [self.entries[0]]
        
        new_entries = verstappen_entries
